...
int min_partition(string s, int i, int j)
{
    ...
    for (int k = i; k < j; k++)
    {
        ...
        if(dp[i][k] == -1 and dp[k+1][j] == -1)
            tempans = min_partition(s, i, k) + min_partition(s, k + 1, j) + 1;
        else if(dp[i][k] == -1 and dp[k+1][j] != -1)
            tempans = min_partition(s,i,k) + dp[k+1][j]+1;
        else if(dp[i][k] != -1 and dp[k+1][j] == -1)
            tempans = dp[i][k] + min_partition(s,k+1,j) + 1;
